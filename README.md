...
Each of Dragon's first four modules currently gather data by web-scraping TrenchyBot, Trench Radar, and Bubblemaps. The challenge is to build a pipeline that connects the Sniper Analysis module with a Solana RPC (ie. [Helius](https://www.helius.dev)) to replace all scrapes. If any data can not be retrieved from the RPC, the developer can use whatever means necessary given the goals stated in [Bounty Details](#bounty-details) below.
...
│   │   └── sniperAnalysis.js   # Module for Sniper Analysis (Helius RPC) # MAKE CHANGES IN THIS FILE
│   ├── telegram
